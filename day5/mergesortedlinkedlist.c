...
SinglyLinkedListNode* mergeLists(SinglyLinkedListNode* head1, SinglyLinkedListNode* head2) {
    ...
    n=length(head1);
    m=length(head2);
    SinglyLinkedList *result=malloc(sizeof(SinglyLinkedList));
    result->head=NULL;
    result->tail=NULL;
    int i,j;
    i=0;
    ...
    while(i<n && j<m){
        if(temp1->data<temp2->data){
            insert_node_into_singly_linked_list(&result, temp1->data);
            i++;
            temp1=temp1->next;
        }
        else{
            insert_node_into_singly_linked_list(&result, temp2->data);
            j++;
            temp2=temp2->next;
        }
